...
try:
    ...
    df = myQuery
    msg0 = 'Benchmark server software data fetched Successful'
    logger.info(msg0)
    ...
    try:
        df.to_sql(env_mixin.SOMANS_BENCHMARK_SVRS, env_mixin.engine_idap, if_exists='append', index=False)
        msg1 = 'Benchmark server software data inserted Successful'
        logger.info(msg1)
        dfLog['status'] = 'Success'
        ...
        try:
            ...
            df.to_sql(env_mixin.SOMANS_BENCHMARK_SVRS_HIST, env_mixin.engine_idap, if_exists='append', index=False)
            msg2 = 'Benchmark history server software data Successful'
            logger.info(msg1)
            dfLog['status'] = 'Success'
