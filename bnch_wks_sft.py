...
logger = env_mixin.logservice.setup_logger('bnch_wks_sft', logfile)

logVar = {'project_name': 'SOMANS', 'report_tag': 'bnch_wks_sft', 'status': 'Success', 'job_date': datetime.today().strftime('%Y-%m-%d'),
...
try:
    ...
    try:
        df.to_sql(env_mixin.SOMANS_BENCHMARK_WKS, env_mixin.engine_idap, if_exists='append', index=False)
        msg1 = 'Benchmark workstation software data inserted Successful'
        logger.info(msg1)
        dfLog['status'] = 'Success'
